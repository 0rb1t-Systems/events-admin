...
/**
 * Status-change form only — admins cannot edit business/contact/email here.
 */
const OrganizerForm: React.FC<OrganizerFormProps> = ({ organizer, onClose }) => {
    ...
    const suspendMutation = useMutation({
        ...
        onError: (error: Error) => toast.error(error.message || "Failed to suspend"),
    });
    ...
    const reactivateMutation = useMutation({
        ...
        onError: (error: Error) => toast.error(error.message || "Failed to reactivate"),
    });
    ...
    return (
        <div className="space-y-4">
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm dark:border-[#1b2e4b] dark:bg-[#0e1726]">
                ...
                <p className="text-gray-600 dark:text-gray-300">{organizer.contact_name}</p>
                <p className="text-gray-500 dark:text-gray-400">{organizer.email}</p>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
                Admins can suspend or reactivate only. Business and contact details are
                managed by the organizer on the Web App.
            </p>
